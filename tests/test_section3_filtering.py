...
def test_section3_filtering():
    ...
    print("=" * 80)
    confident_picks = confident_picks_after_dropna[confident_picks_after_dropna['spread_cover_probability'] > 0.5].copy()
    ...
    print(f"  - After dropping NaN: {len(confident_picks_after_dropna)} rows (removed {rows_removed_by_nan} rows with NaN)")
    rows_removed_by_filter = len(confident_picks_after_dropna) - len(confident_picks)
    print(f"  - After filtering for spread_cover_probability > 0.5: {len(confident_picks)} rows (removed {rows_removed_by_filter} rows)")
